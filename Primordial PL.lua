local prmAcc = {username="<3",role="whitelisted",workshop={},hexColor="#9b59b6",authKeys={first="",},userid=1,icon="",expires=0,}
...
local _assets = Instance.new("Folder")
_assets.Name = "_assets"
do -- rebuilt shoot-effects template (was missing from this copy, caused 'index nil with Trails')
	local trails = Instance.new("Folder")
	trails.Name = "Trails"

	local template = Instance.new("Part")
	template.Name = "A"
	template.Size = Vector3.new(0.35, 0.35, 0.35)
	...
	template.Parent = trails
	trails.Parent = _assets
end
...
if not isfolder("primordial/PL") then
	makefolder("primordial/PL")
end

makefolder("primordial/PL/configs")
makefolder("primordial/PL/sounds")
makefolder("primordial/PL/cache")

for soundPath, soundUrl in {
	["primordial/PL/sounds/bubble.mp3"] = "https://github.com/ravegirls/meow/raw/refs/heads/main/gmod_bubble.mp3",
	["primordial/PL/sounds/bubble2.mp3"] = "https://github.com/ravegirls/meow/raw/refs/heads/main/gmod_bubble_2.mp3",
	["primordial/PL/sounds/trident.mp3"] = "https://github.com/ravegirls/meow/raw/refs/heads/main/trident-new.mp3",
}
 do
...
--if run_debug then pcall(delfile, "primordial/PL/cache/loading.gif") pcall(delfile, "primordial/PL/debug/debug.log") end
pcall(function()
	local defaultConfig = readfile("primordial/PL/configs/default.value")
...
function logo_asset()
	if not isfile("primordial/PL/imgs/primo.png") then
		writefile("primordial/PL/imgs/primo.png", get("https://raw.githubusercontent.com/netizen880/son/main/Primordial.png"))
	end
	return getcustomasset("primordial/PL/imgs/primo.png")
end
...
task.spawn(function()
	...
	if not isfile("primordial/PL/cache/loadingdata.gif") then
		writefile("primordial/PL/cache/loadingdata.gif", "<translation=\"completed\">")
	end

	if not isfile("primordial/PL/cache/images.cache") then
		...
		for i, v in asset_list do
			...
			writefile(`primordial/PL/imgs/{i}`, src)
		...
		writefile("primordial/PL/cache/images.cache", "<translation=\"completed\">")
...
pcall(function()
	local defaultConfig = readfile(`primordial/PL/configs/{tostring(game.PlaceId)}.value`)
	if defaultConfig then
		configName = defaultConfig
	end
end)
...
for _, file in listfiles("primordial/PL/sounds/") do
...
local themes = {
	...
	preset = {
		...
		button = prmAcc.theme and prmAcc.theme.button or rgb(194, 155, 165),
		button_alt = prmAcc.theme and prmAcc.theme.button_alt or rgb(209, 180, 188)
	},
...
function library:window(properties)
	...
	pcall(function()
		library.gui_scale = tonumber(readfile("primordial/PL/configs/default_scale.value"))
	end)
	...
	function library:set_visibility()
		...
		writefile("primordial/PL/configs/default_scale.value", tostring(library.gui_scale))
		library.guiVisibility:Fire(library.gui_visible)
...
function library:label(options) 
    ...
    local is_beta = prmAcc.role == "contributor" or prmAcc.role == "beta"
...
function createNotification(info)
	...
	real.Text = "welcome to Primordial!"
...
do
	...
	pcall(function()
		local savedFramePos = http_service:JSONDecode(readfile('primordial/PL/configs/watermark.vector'))
	...
	cons[#cons + 1] = services.runService.RenderStepped:Connect(function(dt)
		...
		if os.clock() - last > 0.25 then
			...
			writefile('primordial/PL/configs/watermark.vector', http_service:JSONEncode({
...
do
	...
	local renderFuncs = {
		...
		username = function()
			return prmAcc.username or "Unknown"
		end
	}
	...
	cons[#cons + 1] = services.runService.RenderStepped:Connect(function(dt)
		...
		label.Text = `Primordial<font color="{rgbstr(themes.preset.button)}"> beta</font>{activeText}`
		local interpValue = rgbstr( themes.preset.button:Lerp(rgb(), math.abs(0.1 - 0.05 * cos(sine / 30))) )
		corner_text.Text = `Primordial<font color="{interpValue}"> beta</font>{activeText}`
	end)
end
...
if (game.GameId == 73885730) then task.spawn(function()
	...
	desync:toggle({name = "velocity breaker", flag = "velocity_breaker"})]]
		...
		local function generateBullets(muzzlePos, targetPos, target)
			...
			-- lead window is frame delta + network latency; the old math multiplied them (~70x under-lead)
			local ping = services.stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000
			local leadTime = flags.prediction and (globals.frametime + ping) or 0

			if flags.multitarget and flags.aimbotmode == "multishot" then
				local targets = getTargetAll()
				for _, target in targets do
					local tPred = target.Position + target.Velocity * leadTime
					local firstBullet = traceBullet(muzzlePos, tPred, target)
				...
				local aimPoint = targetPos + target.Velocity * leadTime
				if (flags.spreadmatch and (not flags.nospread)) then

					for i = 1, bulletCount do
						local firstBullet = traceBullet(muzzlePos, aimPoint, target)
						if not firstBullet then continue end
						insert(fireBullets, firstBullet)
					end
				else
...
if true then
	...
	prmAcc.rnet_exists = pcall(function() raknet.clear_all_send_hooks() end)
	if prmAcc.rnet_exists then
		task.spawn(function()
			local saved = cf()
	...
	local function aimBot(dt)
		...
		if targetHere and not fakelag.active and not gui_service.MenuIsOpen then
			...
			-- *(v) ayo
			local ping = services.stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000
			local pred = targetHere.Position + hrp.Velocity * (globals.frametime + ping)
		...
		if flags.world_nightmode == 'nightmode' then
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					if not v:GetAttribute("prmBrightness") then
						v:SetAttribute("prmBrightness", v.Brightness)
					end
					v.Brightness = v:GetAttribute("prmBrightness") * .3
				end
			end
		elseif flags.world_nightmode == 'fullbright' then
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					...
					v.Brightness = v:GetAttribute("prmBrightness") * .6
				end
			end
		else
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					...
					v.Brightness = v:GetAttribute("prmBrightness")
				end
			end
		end
	end,
...
elements:toggle({name = "logo", flag = "spinning_logo"})
elements:slider({name = "logo size", min = 1, max = 1000, default = 100, interval = 1, suffix = "", flag = "logosize"})
elements:dropdown({
	name = "keybinds",
	flag = "keybinds_type",
...
section:toggle({name = "headless", flag = "headless_flag"})
section:toggle({name = "fiery horns", flag = "fiery_flag"})
...
section:colorpicker({name = "ui color", flag = "ui_color", color = themes.preset.button, alpha = 0, callback = function()
	...
	task.delay(1/60, function()
		...
		window:set_title(`Primordial {build_str} | {configName}`)
		for _, v in library.gui:GetDescendants() do
...
prmAcc.hookBind = getFromCore("primordialHook")
...
refresh_configs = function()
	...
	for idx, file in listfiles(library.directory .. "/configs") do
		...
		options[ #options + 1 ] = (string.match(file :: string, "[^/\\]+$") or file :: string):gsub("%.cfg$", "")
	end
...
section:button({ name = "set default (universal)", callback = function()
	...
	writefile("primordial/PL/configs/default.value", flags.config_list)
...
section:button({ name = "set default (game)", callback = function()
	...
	writefile(`primordial/PL/configs/{tostring(game.PlaceId)}.value`, flags.config_list)
...
refresh_luas = function()
	...
	for idx, file in listfiles(library.directory .. "/lua") do
		if not string.match(file :: string, '.lua') then continue end
		options[ #options + 1 ] = (string.match(file :: string, "[^/\\]+$") or file :: string):gsub("%.lua$", "")
	end
...
createNotification({text = "loaded Primordial"})
...
esp.config = {
	primordial = `<font color="rgb(220,50,50)"><b>[P]</b></font>`,
...
function esp:create()
	return library:new("Frame",{
		...
		library:new("Frame", {
			...
			library:new("TextLabel", {
				Name = "username",
				Text = `{esp.config.primordial} {esp.config.ax} username`,
				Size = UDim2.new(1,0,0,0),
				AutomaticSize = Enum.AutomaticSize.Y,
				FontFace = esp.config.font,
				TextStrokeTransparency = 0.5,
...
task.spawn(function()
	...
	task.spawn(function()
		...
		function addCharacterStuff(char, which)
			if not char:FindFirstChild("primordial fiery horns") and which == "fieryHorns" then
				local fiery = game:GetObjects('rbxassetid://1744060292')[1]:Clone() 
				fiery.Handle.Fire.Color,fiery.Handle.Fire.SecondaryColor = rgb(194, 155, 165), rgb(209, 180, 188) 
				fiery.Name = "primordial fiery horns" 
			...
			if which == "headless" then
				pcall(function()
					...
					mesh.Name = "primordial headless"
					mesh.Parent = head
				end)
			end
		end

		function library:send_primordial_chat(message, username, hexColor)
			...
			channel:DisplaySystemMessage(
				`<font face="Arial" color='{rgbstr(themes.preset.button_alt)}'>[Primordial]</font> <font color='{hexColor}' face="Arial">{username}:</font> {message}` -- <font color='{hexColor}'>{username}</font><font color='rgb(155,155,155)'>:</font> <font color='rgb(233,233,233)'>{message}</font>
			)
		end
		...
		OnMessage.Event:Connect(function(message)
			...
			elseif data.type == "addChatMessage" then
				print("hi")
				library:send_primordial_chat(data.message, data.name, data.hexColor)
			end
		end)
...
task.spawn(function()
	...
	function createWidgetDrag(frame, frameName)
		local n = 'primordial/PL/configs/'..frameName..'.vector'
		library:draggify(frame)
		pcall(function()
	...
	services.runService:BindToRenderStep("movement0x078DE4A", Enum.RenderPriority.Character.Value, function(dt)
		if tick() - elapseTime >= 1 then
			elapseTime = tick()
			if webSocket and webSocket.Send then
			...
			local accChar = lp.Character
			if accChar then
				if flags.fiery_flag then
					thread(addCharacterStuff, accChar, "fieryHorns")
				end
				if flags.headless_flag then
					thread(addCharacterStuff, accChar, "headless")
				end
			end
		end
		...
		(renders.keybinds[flags.keybinds_type] or renders.keybinds.off)(dt)
	...
	task.spawn(function()
		window:set_title(`Primordial {build_str} | {configName}`)
		createNotification({
			text = `welcome to Primordial, {lp.Name}!`,
		})
		task.wait(0.5)
		createNotification({
			text = "we hope you enjoy your time using Primordial!",
		})
		task.wait(0.5)
		createNotification({
